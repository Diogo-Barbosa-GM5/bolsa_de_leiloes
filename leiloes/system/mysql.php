<?
	...
	class Mysql  {
		...
		//Altera de protected para public por Diogo - 09/09/2019
		public $db;
        ...
        public function __construct() {
			...
			try {
				$this->db = new PDO('mysql:host='.$localhost_config.';dbname='.$banco_config, $nome_config, $senha_config,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
		...
		private function logs_acoes($tabela, $id, $acao, $campo){
			if($id and LUGAR != 'site' AND $tabela!='log_acoes' AND $tabela!='z_txt' AND isset($_SESSION['x_'.LUGAR]->id) AND $_SESSION['x_'.LUGAR]->id!=1){
				...
				$sql = $this->db->prepare(" INSERT INTO `log_acoes`	(`lang`,	`acoes`,	`usuarios`,	`usuarios_id`,	`tabelas`,	`item_id`,	`item`,	`ip`,	`id`)
											VALUES 					( ?,		 ?,			 ?,			 ?,				 ?,			 ?,			 ?,		 ?,		 ?  )");
