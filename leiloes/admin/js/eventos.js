			...
			$.ajax({
				...
				success: function($json){
					...
					$("table.AOVIVO_leilao_"+$id).each(function() {
						setTimeout(function(){ leilao_aovivo($id); }, 1000);
					});
					...
					if($json.erro != null){
						...
						// Lista
							...
							if($acao == 'lista' && $_SESSION['filtro_avancado['+$modulo+']']){
								...
								$(".dataTable").selectable({ filter: "tr", selected: function(event, ui){ datatable_button_disabled(); } });
								$('.dataTable tbody').on('dblclick', 'tr', function () { views($modulo, 'edit', $gets); });
	...
	// SELECT2
		...
		function select2_select(e, ev){
			...
			if($val == '(gi)'){
				...
				setTimeout(function(){ $(".carregando").hide(); }, 500);
				zerar = 1;
			}
			if(zerar)
	...
	// DATATABLE
		...
		function datatable_update($tipo, $id){
			if($('table.datatable').html()){
				if($tipo == 'row')	$.atualizar_datatable_row($id);
				else				$.atualizar_datatable();
			};
