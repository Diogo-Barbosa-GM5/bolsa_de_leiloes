...
//if(extension_loaded('zlib')){ob_start('ob_gzhandler');}
	...
	function __autoload($class_name){
		...
		foreach ($_POST['col'] as $key => $value) {
			...
			switch ($cols[$key][0]){
			    case 'mais_fotos':			$cols[$key][0] = 'id';		$cols[$key][1] = 'mais_fotos';			break;
			    case 'mais_comentarios':	$cols[$key][0] = 'id';		$cols[$key][1] = 'mais_comentarios';	break;
			}
		}
						...
						// COLUNAS
							// ACOES
								if($col[1] == 'acoes'){
					                ...
					                if($bloquear) 																				$return .= '<a onclick="datatable_acoes('.A.'block'.A.', '.A.$modulos->id.A.', '.A.$value->id.A.')"		class="dib p5" 	title="Ativar ou Bloquear Item">   				  <i class="di fz16 fa fa-check '.iff($dt_value->status, 'c_verde', 'n_ativo').'"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-star-)', $modulos->informacoes))			$return .= '<a onclick="datatable_acoes('.A.'star'.A.', '.A.$modulos->id.A.', '.A.$value->id.A.')"  		class="dib p5" 	title="Selecione os Itens como Destaque">		  <i class="di fz16 fa fa-star '.iff($dt_value->star, 'c_amarelo', 'n_ativo').'"></i> </a> ';
									...
									if($modulos->id == '63'):
											...
											if($leilao->data_fim <= date('c')):
										$return .= '<a href="/arrematado_dados.php?leilao='.$value->id.'" onclick="window.location.href=\'/arrematado_dados.php?leilao='.$value->id.'\';" target="_blank" class="dib p5" title="Pegar Relatorios">		 
										 <i class="di fz16 fa fa-file c_verde" ></i> </a> ';

									endif;		
									endif;		


									if(isset($modulos->informacoes) and preg_match('(-lancamentos-)', $modulos->informacoes))	$return .= '<a onclick="datatable_acoes('.A.'lancamentos'.A.', '.A.$modulos->id.A.', '.A.$value->id.A.')"	class="dib p5" 	title="Selecione os Itens como Lançamento">		  <i class="di fz16 fa fa-dot-circle-o '.iff($dt_value->lancamentos, 'c_verde', 'n_ativo').'"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-promocao-)', $modulos->informacoes))		$return .= '<a onclick="datatable_acoes('.A.'promocao'.A.', '.A.$modulos->id.A.', '.A.$value->id.A.')"		class="dib p5" 	title="Mostrar Video Ao Vivo"> 		  			<i class="di fz16 fa fa-youtube-play '.iff($dt_value->promocao, 'c_preto', 'n_ativo').'"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-clonar-)', $modulos->informacoes))		$return .= '<a onclick="datatable_acoes('.A.'clonar'.A.', '.A.$modulos->id.A.', '.A.$value->id.A.')"		class="dib p5" 	title="Clonar Item">  							  <i class="di fz16 c_azul fa fa-files-o ativo"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-mapa-)', $modulos->informacoes))			$return .= '<a onclick="boxs('.A.'mais_mapas'.A.', '.A.'item='.$value->id.'&modulos='.$modulos->id.A.', 1);"		class="dib p5" 	title="Cadastrar uma Localização para esse Item"> <i class="di fz16 c_azul fa fa-globe ativo"></i> </a> ';
									//if($modulos->id == 54){
							...
							// ACOES BOXS
									...
									if($bloquear)																				$return .= '<a onclick="datatable_acoes_boxs('.A.'block'.A.', '.A.$modulos->id.A.', '.A.$_POST['rand'].A.', '.A.$value->id.A.')" 			class="dib fz16 pl5 pr5"	title="Ativar ou Bloquear Item">   			<i class="fa fa-check '.iff($dt_value->status, 'c_verde', 'n_ativo').'"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-star-)', $modulos->informacoes))			$return .= '<a onclick="datatable_acoes_boxs('.A.'star'.A.', '.A.$modulos->id.A.', '.A.$_POST['rand'].A.', '.A.$value->id.A.')" 			class="dib fz16 pl5 pr5"  	title="Selecione os Itens como Destaque">	<i class="fa  fa-star '.iff($dt_value->star, 'c_amarelo', 'n_ativo').'"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-lancamentos-)', $modulos->informacoes))	$return .= '<a onclick="datatable_acoes_boxs('.A.'lancamentos'.A.', '.A.$modulos->id.A.', '.A.$_POST['rand'].A.', '.A.$value->id.A.')" 	class="dib fz16 pl5 pr5"	title="Selecione os Itens como Lançamento">	<i class="fa  fa-dot-circle-o '.iff($dt_value->lancamentos, 'c_verde', 'n_ativo').'"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-promocao-)', $modulos->informacoes))		$return .= '<a onclick="datatable_acoes_boxs('.A.'promocao'.A.', '.A.$modulos->id.A.', '.A.$_POST['rand'].A.', '.A.$value->id.A.')" 		class="dib fz16 pl5 pr5"	title="Selecione os Itens como Promoção"> 	<i class="fa  fa-certificate '.iff($dt_value->promocao, 'c_azul', 'n_ativo').'"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-clonar-)', $modulos->informacoes))		$return .= '<a onclick="datatable_acoes_boxs('.A.'clonar'.A.', '.A.$modulos->id.A.', '.A.$_POST['rand'].A.', '.A.$value->id.A.')" 		class="dib fz16 pl5 pr5"	title="Clonar Item">  						<i class="c_azul fa fa-files-o ativo"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-edit-)', $modulos->informacoes))			$return .= '<a onclick="datatable_campos_boxs('.A.$modulos->id.A.', '.A.$_POST['rand'].A.', '.A.$value->id.A.')" 					class="dib fz16 pl5 pr5"	title="Editar Item"> 						<i class="c_cinza fa fa-edit (alias) ativo"></i> </a> ';
									if(isset($modulos->informacoes) and preg_match('(-excluir-)', $modulos->informacoes))		$return .= '<a onclick="if(confirm('.A.'Deseja realmente deletar este item?'.A.'))datatable_acoes_boxs('.A.'delete'.A.', '.A.$modulos->id.A.', '.A.$_POST['rand'].A.', '.A.$value->id.A.')" class="dib fz16 pl5 pr5 c_vermelho"> <i class="fa fa-times" title="Deletar Item"></i> </a> ';
									if(!$key) $return .= '<script> $(document).ready(function() { $(".datatable_boxs tbody td .td_datatable").on("dblclick", function(){ datatable_campos_boxs_selecionar_select2('.A.$_POST['rand'].A.', this); javascript:fechar_all(); }) }); </script> ';
								...
								// Imagem
									...
									if($item and file_exists('../../../../web/fotos/'.$item)){
										if(preg_match('(.swf)', strtolower($item))){
											...
											$foto = 'web/fotos/'.$nome_da_foto['nome'].'.'.$nome_da_foto['ext'];
