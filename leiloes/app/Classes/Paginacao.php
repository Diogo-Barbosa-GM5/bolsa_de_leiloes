			...
			// PAGINACAO
				...
				if(!$itens_pag){
					...
					$dados['pagg'] .= '<div class="pagg">';
						$dados['pagg'] .= '<a '.iff($_GET['pag'], 'href="'.$url_atual.'pag=0" class="', 'class="ativo').'" ><</a>';
						for($y; $y<=$num_final_pag[0]; $y++){
							if($n_paginas <= 9){ $n_paginas++;
								$dados['pagg'] .= '<a '.iff($_GET['pag']!=$y, 'href="'.$url_atual.'pag='.$y.'" class="', 'class="ativo').'" >'.($y+1).'</a>';
							}
						}
						$dados['pagg'] .= '<a '.iff($_GET['pag']!=($y-1), 'href="'.$url_atual.'pag='.($y-1).'" class="', 'class="ativo').'" >></a>';
					$dados['pagg'] .= '</div>';
				}
