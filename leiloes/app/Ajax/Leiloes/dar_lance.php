		...
		// Outras Verificacoes
			...
			// VERIFICAR ERROS
				...
				// Verificar se Leilao ja Finalizou
					...
					$arr['erro'][] = lang('Seu Lance tem q ser maior que o lance atual!');

				} elseif($_POST['lance'] < $lotes->lance_ini){
					$arr['erro'][] = lang('O Lance ínicial é de').' '.preco($lotes->lance_ini, 1).'!';
				
				} elseif($_POST['lance'] < $lotes->lances + '50'){
					$arr['erro'][] = lang('O lance tem que ser no minimo superior a +50 do atual').'!';
				}
			// VERIFICAR ERROS
			...
			if(!isset($arr['erro'])){
				...
				// ATUALIZANDO ACRESCIMO
					if($lotes->data_acrescimo_ok){
						...
						$mysql->campo['data_acrescimo'] = date('Y-m-d H:i:s', mktime(date('H'), date('i'), date('s')+$acrescimo, date('m'), date('d'), date('Y')));;
				...
				// MUDAR LANCE DA TABLE LOTES PARA LOTES_LANCES Mudar lance da table lote para lotes_lances
					...
					unset($mysql->campo);
					$mysql->logs = 0;
					$mysql->campo['lances_data'] = date('Y-m-d H:i:s');
